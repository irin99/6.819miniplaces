my_name = "Irin Ghosh"
my_teammate = "Lay Jain" # Fill in if you have a teammate, leave blank otherwise
my_team_name = "Elementary" # Your team name on the submission server

print("I'm %s. I worked with %s. I'm on team %s" % (my_name, my_teammate, my_team_name))


import os
data_dir = "/u/irin/miniplaces/data"
...
def get_dataloaders(input_size, batch_size, shuffle = True):
    ...
    data_transforms = {
        ...
        'train3': transforms.Compose([
            transforms.ColorJitter(brightness=(0.25,0.75), contrast=(0.25,0.75), 
               saturation=(0.25,0.75), hue=(-0.15,0.15)),
            transforms.Resize(input_size),
            transforms.CenterCrop(input_size),
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ...
    image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms[x]) for x in ['test','val']}
    image_datasets['train1'] = datasets.ImageFolder(os.path.join(data_dir,'train'), data_transforms['train1'])
    image_datasets['train2'] = datasets.ImageFolder(os.path.join(data_dir,'train'), data_transforms['train2'])
    image_datasets['train3'] = datasets.ImageFolder(os.path.join(data_dir,'train'), data_transforms['train3'])
    # Create training and validation dataloaders
    # Never shuffle the test set
    dataloaders_dict = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=batch_size, shuffle=False if x != 'train' else shuffle, num_workers=4) for x in ['test','val']}
    dataloaders_dict['train'] = torch.utils.data.DataLoader(torch.utils.data.ConcatDataset([image_datasets['train1']
    ,image_datasets['train2'],image_datasets['train3']]),  batch_size=batch_size, shuffle=shuffle)
...
def train_model(model, dataloaders, criterion, optimizer, save_dir = None, save_all_epochs=False, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            np.savetxt("foo.csv", np.asarray([epoch_loss, epoch_acc]), delimiter=",")
...
def make_optimizer(model):
    ...
    #optimizer = optim.SGD(params_to_update, lr=0.02, momentum=0.8, nesterov=True)
    optimizer = optim.Adam(params_to_update)
    #optimizer = optim.RMSprop(params_to_update)
    return optimizer
...
np.savetxt("foo.csv", np.asarray([16,24,48]), delimiter=",")
...
output_test_labels = "test_set_predictions_finall"
